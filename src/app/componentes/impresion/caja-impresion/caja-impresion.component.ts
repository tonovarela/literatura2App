import { HttpClient } from '@angular/common/http';


import { Component, ElementRef, Input, OnInit, ViewChild, AfterViewInit } from '@angular/core';

import pdfMake from "pdfmake/build/pdfmake";
import pdfFonts from "pdfmake/build/vfs_fonts";
import * as printJS from 'print-js';
import { InlineWorker } from 'src/app/class/InlineWorker';
import { Impresion } from 'src/app/interfaces/impresion.interface';
pdfMake.vfs = pdfFonts.pdfMake.vfs;
@Component({
  selector: 'app-caja-impresion',
  templateUrl: './caja-impresion.component.html',
  styles: [
  ]
})
export class CajaImpresionComponent implements OnInit, AfterViewInit {
  @ViewChild("container") container: ElementRef;
  @Input("print") print: Impresion;
  

  base64string: string = "";
  cuerpo = []
  cargando: boolean = false;

  constructor(private http: HttpClient) { }
  ngAfterViewInit(): void {
    this.cargando = true;
    this.http.get(this.print.url).subscribe(response => {
      const targetElement = this.container.nativeElement;
      const pdf = `${response["pdf"]}`;
      const iframe = document.createElement('iframe');
      iframe.style.width = "100%"
      iframe.style.height = "800px";
      iframe.src = pdf;
      targetElement.appendChild(iframe);
      this.base64string = pdf.replace("data:application/pdf;base64,", "");
      this.cargando = false;
    });

  }
  ngOnInit(): void {



  }


  imprimir() {
    printJS({ printable: `${this.base64string}`, type: 'pdf', base64: true });
  }

  result: number = 0;
  obtenerArchivo() {
    this.cargando = true;


    const worker = new InlineWorker(() => {
      // START OF WORKER THREAD CODE}


      const Template = () => {
        const logoLito = "";
        return [
          // {
          //   image: `data:image/png;base64,${logoLito}`,
          //   fit: [100, 100],
          //   alignment: 'center',
          //   margin: [0, 0, 0, 20]
          // },
          {
            text: 'Contiene 21  kits',
            fontSize: 10,
            characterSpacing: 5,
            alignment: 'center',
            color: '#FFFFFF',
            background: "black"
          },
          {
            text: 'Customer Details',
            style: 'sectionHeader'
          },
          {
            columns: [
              [
                { text: "Direccion", style: 'textoNormal' },
                { text: "Email", style: 'textoNormal' },
                { text: "Contacto", style: 'textoNormal' },
              ],
              [
                {
                  text: `Date: ${new Date().toLocaleString()}`,
                  alignment: 'right',
                  style: 'textoNormal'
                },
                {
                  text: `Bill No : ${((Math.random() * 1000).toFixed(0))}`,
                  alignment: 'right',
                  style: 'textoNormal'
                }
              ]
            ]
          },
          {
            text: 'Order Details',
            style: 'sectionHeader',
            margin: [0, 0, 0, 40],
          },
          {
            text: "",
            margin: [0, 0, 0, 10],
            style: 'textoNormal'
          },
          // {
          //   columns: [
          //     [{
          //       image:code ,//await obtenerMatrix(),
          //       width: 250,
          //       alignment: 'center',
          //       margin: [0, 0, 0, 5]
          //     }],
          //   ]
          // },
          {
            text: "CAJA",
            fontSize: 50,
            bold: true,
            alignment: 'center',
            margin: [0, 50, 0, 10],
            color: '#047886'
          }
        ];

      }


      const crearCuerpoPDF = (limit) => {
        let cuerpo = []
        const totalPaginas = limit;
        for (let i = 0; i < totalPaginas; i++) {
          cuerpo.push(Template(), { text: '', pageBreak: i < (totalPaginas - 1) ? 'before' : '' });
        }

        // @ts-ignore
        this.postMessage({
          datos: cuerpo
        });
      };

      // @ts-ignore
      this.onmessage = (evt) => {
        //  console.log('Calculation started: ' + new Date());        
        crearCuerpoPDF(evt.data.limit);

      };
      // END OF WORKER THREAD CODE
    });

    worker.postMessage({ limit: 2, });

    worker.onmessage().subscribe((message) => {


      let docDefinition = {
        pageSize: 'A4',
        pageOrientation: 'landscape',
        compress: true,
        content: message.data.datos,
        footer: function (currentPage, pageCount) {
          return [, {
            text: currentPage.toString() + ' de ' + pageCount,
            bold: true,
            fontSize: 20,
            alignment: 'center',
            margin: [10, 10, 10, 10]
          }];
        },
        styles: {
          textoNormal: {
            fontSize: 8,
          },
          sectionHeader: {
            bold: true,
            decoration: 'underline',
            fontSize: 8,
            margin: [0, 0, 0, 0]
          }
        }
      };
      const pdf = pdfMake.createPdf(docDefinition);

      pdf.getDataUrl((dataUrl) => {
        console.log(dataUrl);
        const targetElement = this.container.nativeElement;
        const iframe = document.createElement('iframe');
        iframe.style.width = "100%"
        iframe.style.height = "800px";
        iframe.src = `${dataUrl}`;
        targetElement.appendChild(iframe);
        this.print = dataUrl.replace("data:application/pdf;base64,", "");
        this.cargando = false;

      });
      //pdf.open();
      worker.terminate();
    });

    worker.onerror().subscribe((data) => {
      console.log(data);
    });

  }

}


